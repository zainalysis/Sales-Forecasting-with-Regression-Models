...
# Scale the feature importances to make them more visible
scaled_importance = feature_importance * 1000000

# Create the plot
plt.figure(figsize=(20, 10))
bars = plt.bar(range(len(scaled_importance)), scaled_importance[sorted_idx], color='skyblue')

# Annotate the bars with the exact scaled values
for bar, value in zip(bars, scaled_importance[sorted_idx]):
    plt.text(bar.get_x() + bar.get_width() / 2, bar.get_height() - 5, f'{value:.2f}',
             ha='center', va='bottom', fontsize=10, color='black', fontweight='bold')

# Customize the x-axis with feature names
plt.xticks(range(len(scaled_importance)), np.array(X_train_final.columns)[sorted_idx], rotation=90)
